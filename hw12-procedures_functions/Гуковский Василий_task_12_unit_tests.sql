...
begin
  v_payment_id := create_payment(p_payment_detail => v_payment_detail,
                                 ...
                                 p_from_client_id => 1,
                                 p_to_client_id => 2,
                                 p_create_timestamp => systimestamp);
  dbms_output.put_line('Payment_id: '|| v_payment_id);
  
end; 
/

select * from payment p where p.payment_id = 68;
select * from payment_detail pd  where pd.payment_id = 68 order by pd.field_id;

-- Проверка процедуры "Сброс платежа в "ошибочный статус""
declare 
v_payment_id payment.payment_id%type := 68;
...
begin
 delete_payment_detail(p_payment_id => v_payment_id,p_delete_payment_detail => v_delete_payment_detail); 
end;
/
